...
<div class="row">
    ...
    <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 sub-title-section">
        <h2 class="text-center">Tratamientos realizados  (Guardados = {{isset($measurements) && count($measurements) > 0? count($measurements): 0}})</h2>
    </div>
    <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <button class="btn  btn-primary btn-new-row" id="new" type="button" onclick="getModalValues()">Nuevo</button>
            <div class="table-responsive">
                <table class="table table-bordered" id="againstReferenceTable">
                    ...
                    <tbody>
                        @if(isset($measurements) && count($measurements) > 0) @foreach($measurements as $item)
                        <tr style="background-color:#bbb;">
                            <td>{{$item->conduit}}</td>
                            <td>{{$item->measuring}}</td>
                            <td>{{$item->reference}}</td>
                            <td>{{$item->lima}}</td>
                            <td class="text-center">{!!Form::checkbox('measurementsID[]',$item->id, false,['class' => 'form-control'])!!}</td>
                        </tr>
                        @endforeach @endif
                    </tbody>
                </table>
    ...
    <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 sub-title-section">
        <h2 class="text-center">Archivos =  (Guardadas = {{isset($imgAgainstReference) && count($imgAgainstReference) > 0? count($imgAgainstReference): 0}})</h2>
    </div>
                    ...
                    <div class="container-delete-image text-center">
                        <div class="form-check">
                            <label class="form-check-label container-delete-image">
                                <input type="checkbox" class="form-check-input" name="imgDeleteID[]" value="{{$item->src}}">Eliminar
                            </label>
                        </div>
                    </div>
                </div>
    ...
    @endif
    <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
        <div class="btn-group container-input-file btn-group-files">
            <label id="labelInputFile" onclick="callInputFile()" class="btn"><i class="fa fa-file-image-o"></i></label>
            <input id="files" class="files" type="file" name="file[]" multiple="multiple" accept="image/x-png,image/gif,image/jpeg,image/PNG" />
            <button type="button" class="btn" id="clearFiles" onclick="removeFiles()"><i class="fa fa-trash"></i></i>
            </button>
        </div>
    </div>
